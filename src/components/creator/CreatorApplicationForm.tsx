...
export const CreatorApplicationForm = () => {
  ...
  const validate = (currentStep: number) => {
    ...
    if (currentStep === 1 || currentStep === 3) {
      check(formData.roleTags.length > 0, 'roleTags', 'Choose at least one role.');
      check(formData.formatTags.length > 0, 'formatTags', 'Choose at least one format or platform.');
      check(Boolean(formData.experienceLevel), 'experienceLevel', 'Choose your experience level.');
    }
    if (currentStep === 2 || currentStep === 3) check(formData.availabilityTags.length > 0, 'availabilityTags', 'Choose at least one availability preference.');
