...
export const CompanyBriefForm = () => {
  ...
  const validate = (currentStep: number) => {
    ...
    if (currentStep === 0 || currentStep === 3) {
      check(data.companyName.trim().length >= 2, 'companyName', 'Enter the company name.');
      check(data.contactName.trim().length >= 3, 'contactName', 'Enter the contact person’s full name.');
      check(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email.trim()), 'email', 'Enter a valid email address.');
      check(/^\+?[0-9\s-]{10,18}$/.test(data.phone.trim()), 'phone', 'Enter a valid phone or WhatsApp number.');
      check(isUrlOrEmpty(data.websiteUrl), 'websiteUrl', 'Use a complete http(s) link, or leave this blank.');
      check(Boolean(data.businessCategory), 'businessCategory', 'Choose a business category.');
    }
    if (currentStep === 1 || currentStep === 3) {
      check(data.goals.length > 0, 'goals', 'Choose at least one goal.');
      check(data.platforms.length > 0, 'platforms', 'Choose at least one platform.');
    }
    if (currentStep === 2 || currentStep === 3) check(data.services.length > 0, 'services', 'Choose at least one service.');
    if (currentStep === 3) {
      check(data.talentNeeds.length > 0, 'talentNeeds', 'Choose a talent need or ask us to recommend one.');
      check(Boolean(data.budgetRange), 'budgetRange', 'Choose an estimated budget.');
      check(Boolean(data.timeline), 'timeline', 'Choose a timeline.');
      check(Boolean(data.usageRights), 'usageRights', 'Choose a usage-rights need.');
      check(data.description.trim().length >= 10, 'description', 'Tell us a little more about the campaign.');
  ...
  return (
    <form onSubmit={submit} className="rounded-[40px] border border-primary/15 bg-white p-5 shadow-soft sm:p-8">
      ...
      <div className="mt-8">
        {step === 0 && <section className="space-y-5"><div><h3 className="text-xl font-semibold text-espresso">Company basics</h3><p className="mt-1 text-sm text-cocoa">A few details so we can understand the business and reply to the right person.</p></div><div className="grid gap-5 md:grid-cols-2"><label className="text-sm font-medium text-cocoa">Company name<input value={data.companyName} onChange={(event) => setValue('companyName', event.target.value)} autoComplete="organization" className={fieldClass} />{error('companyName')}</label><label className="text-sm font-medium text-cocoa">Contact person<input value={data.contactName} onChange={(event) => setValue('contactName', event.target.value)} autoComplete="name" className={fieldClass} />{error('contactName')}</label><label className="text-sm font-medium text-cocoa">Email<input value={data.email} onChange={(event) => setValue('email', event.target.value)} type="email" autoComplete="email" className={fieldClass} />{error('email')}</label><label className="text-sm font-medium text-cocoa">Phone / WhatsApp<input value={data.phone} onChange={(event) => setValue('phone', event.target.value)} inputMode="tel" autoComplete="tel" placeholder="+91 98..." className={fieldClass} />{error('phone')}</label><label className="text-sm font-medium text-cocoa md:col-span-2">Website, Instagram, or YouTube link <span className="font-normal text-cocoa/70">(optional)</span><input value={data.websiteUrl} onChange={(event) => setValue('websiteUrl', event.target.value)} placeholder="https://..." className={fieldClass} />{error('websiteUrl')}</label><label className="text-sm font-medium text-cocoa md:col-span-2">Business category<select value={data.businessCategory} onChange={(event) => setValue('businessCategory', event.target.value)} className={fieldClass}><option value="">Choose a category</option>{COMPANY_BUSINESS_CATEGORIES.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{error('businessCategory')}</label></div></section>}

        {step === 1 && <section className="space-y-7"><div><h3 className="text-xl font-semibold text-espresso">Goal and platform</h3><p className="mt-1 text-sm text-cocoa">Choose every goal and channel that matters. You can choose more than one.</p></div><div><p className="mb-3 text-sm font-semibold text-espresso">What do you want to achieve?</p><ChoiceGrid options={COMPANY_GOALS} selected={data.goals} onToggle={(value) => toggle('goals', value)} />{error('goals')}</div><div><p className="mb-3 text-sm font-semibold text-espresso">Which platforms should we focus on?</p><ChoiceGrid options={COMPANY_PLATFORMS} selected={data.platforms} onToggle={(value) => toggle('platforms', value)} />{error('platforms')}</div>{showMonetization && <div><p className="mb-3 text-sm font-semibold text-espresso">What are you looking to monetize? <span className="font-normal text-cocoa/70">(optional)</span></p><ChoiceGrid options={MONETIZATION_TARGETS} selected={data.monetizationTargets} onToggle={(value) => toggle('monetizationTargets', value)} /></div>}</section>}

        {step === 2 && <section className="space-y-7"><div><h3 className="text-xl font-semibold text-espresso">Services needed</h3><p className="mt-1 text-sm text-cocoa">Select the help you need. We will shape this into a focused scope rather than sell every service at once.</p></div>{COMPANY_SERVICE_GROUPS.map((group) => <div key={group.title}><p className="mb-3 text-sm font-semibold text-espresso">{group.title}</p><ChoiceGrid options={group.options} selected={data.services} onToggle={(value) => toggle('services', value)} /></div>)}{error('services')}</section>}

        {step === 3 && <section className="space-y-7"><div><h3 className="text-xl font-semibold text-espresso">Talent, budget and timeline</h3><p className="mt-1 text-sm text-cocoa">These details guide recommendations. Final scope, availability, rights, and pricing are always confirmed in writing.</p></div><div><p className="mb-3 text-sm font-semibold text-espresso">What talent do you need?</p><ChoiceGrid options={COMPANY_TALENT_NEEDS} selected={data.talentNeeds} onToggle={(value) => toggle('talentNeeds', value)} />{error('talentNeeds')}</div><div className="grid gap-5 md:grid-cols-2"><label className="text-sm font-medium text-cocoa">Estimated budget<select value={data.budgetRange} onChange={(event) => setValue('budgetRange', event.target.value)} className={fieldClass}><option value="">Choose a range</option>{BUDGET_RANGES.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{error('budgetRange')}</label><label className="text-sm font-medium text-cocoa">When do you want to start?<select value={data.timeline} onChange={(event) => setValue('timeline', event.target.value)} className={fieldClass}><option value="">Choose a timeline</option>{TIMELINES.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{error('timeline')}</label><label className="text-sm font-medium text-cocoa md:col-span-2">Content usage needs<select value={data.usageRights} onChange={(event) => setValue('usageRights', event.target.value)} className={fieldClass}><option value="">Choose an option</option>{USAGE_RIGHTS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{error('usageRights')}</label></div><label className="block text-sm font-medium text-cocoa">Tell us what you are trying to build or sell<textarea value={data.description} onChange={(event) => setValue('description', event.target.value)} className={`${fieldClass} min-h-36`} placeholder="Product, offer, audience, campaign context, and anything else we should know." />{error('description')}</label><div className="rounded-[28px] border border-primary/15 bg-porcelain p-5"><label className="flex gap-3 text-sm leading-6 text-cocoa"><input checked={data.consent} onChange={(event) => setValue('consent', event.target.checked)} type="checkbox" className="mt-1 h-4 w-4 shrink-0 accent-primary" />I agree to the Terms, Privacy Policy, Brand Agreement Summary, Refund Policy, Content Usage Policy, and Safety Policy.</label>{error('consent')}<p className="mt-4 text-xs leading-5 text-cocoa">{COMPANY_INQUIRY_CONSENT}</p><p className="mt-3 flex flex-wrap gap-x-3 gap-y-2 text-xs font-semibold text-primary">{companyAgreementLinks.map((item) => <Link key={item.href} href={item.href} className="underline underline-offset-4">{item.label}</Link>)}</p></div></section>}
      </div>
